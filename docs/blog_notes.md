...
### Step 1 Extension: Probing Classifiers — "표면 특성" 경고

**실험:** 각 레이어의 hidden state에 logistic regression (LOO-CV)

**결과:** 모든 모델, 모든 레이어에서 ~100% 정확도

| 모델 | Peak accuracy | Peak layer | 60%+ 레이어 |
|---|---|---|---|
| klue/bert-base (KO) | 1.00 | L0 | 전체 (0-11) |
| Qwen3-0.6B (KO) | 1.00 | L3 | 전체 (0-27) |
| Qwen3-0.6B (EN) | 1.00 | L0 | 전체 (0-27) |

**Controls:**
1. Permutation test (100x shuffle): 실제 1.00 vs 셔플 0.47±0.19 → p=0.00 (유의)
2. **PCA(1) = 100%**: 단 1개 주성분만으로 완벽 분리

**해석: 이건 "밀도 인코딩"이 아니다.**
고밀도(격언/인용구) vs 저밀도(일상 서술) 문장은 어휘와 문체가 완전히 다르다.
"혁명이 숙적을 주인으로..." vs "오늘 아침에 일찍 일어나서..."
— 임베딩 레이어부터 이미 PCA 1차원으로 구분된다.

**블로그에 쓸 수 있는 서사:**
"트랜스포머가 고밀도 문장을 '감지'할 수 있느냐고? 당연하다 — 하지만 이건 속임수다.
PCA 1차원만으로 100% 분리된다면, 모델은 '밀도'를 보는 게 아니라 '격언 vs 일상문'을 보는 것이다.
이건 마치 '이 AI는 고양이와 개를 구분할 수 있다'고 했는데, 사실 배경색으로 구분하는 것과 같다.
진짜 질문은 '분류할 수 있는가?'가 아니라 '어떻게 처리하는가?'다 — Layer Delta와 Surprisal이 답하는 질문."
...
## Step 2 결과 — Diffusion-like Crystallization

### 방법
Masked LM (BERT) = discrete diffusion denoiser (D3PM, Austin et al. 2021).
전체 [MASK] → confidence 순으로 unmask → entropy/confidence 궤적 추적.

모델: klue/bert-base (KO), bert-base-uncased (EN)

### 결과

| 모델 | 지표 | 방향 | p값 |
|---|---|---|---|
| klue/bert-base (KO) | Mean crystallization | = | 1.00 ns |
| klue/bert-base (KO) | Convergence area | = | 0.67 ns |
| bert-base-uncased (EN) | Mean crystallization | = | 0.67 ns |
| **bert-base-uncased (EN)** | **Convergence area** | **High > Low** | **0.011 \*** |

### 해석

**영어에서만 결정화 가설 지지.**
고밀도 영어 문장이 denoising 과정에서 총 불확실성(entropy 곡선 면적)이 유의미하게 더 높다.
= 모델이 "이 문장을 복원하는 데 더 많은 정보가 필요했다" = 밀도가 높다.

**한국어 null result 해석:**
klue/bert-base의 masked LM head가 한국어 토큰 예측에서는 밀도 차이를 반영하지 못할 수 있음.
또는: 한국어 SOV 구조에서 iterative unmasking이 의미 있는 순서를 만들지 못함
(동사가 마지막에 오는 구조 → 문장 끝이 항상 마지막에 unmask → 밀도 무관).

**Step 1과의 연결:**
- Step 1 surprisal (EN): mean surprisal High > Low, p=0.008** → 고밀도가 예측 어려움
- Step 2 convergence area (EN): High > Low, p=0.011* → 고밀도가 복원 어려움
- 동일한 현상의 다른 관점: "예측 난이도"(decoder) ≈ "복원 난이도"(diffusion)

### 블로그에 쓸 수 있는 서사
"같은 BERT 모델을 두 가지 방식으로 사용했다.
첫째, forward pass로 내부 신호를 관찰 (Step 1) — 고밀도 문장에서 Layer Delta가 더 크다.
둘째, iterative unmasking으로 denoising 과정을 시뮬레이션 (Step 2) — 고밀도 문장이 복원에 더 많은 불확실성을 남긴다.
두 관점이 일치한다: 밀도 높은 문장은 모델에게 더 '어렵다'."

### MDLM/BD3-LM 실제 테스트 결과
MDLM(kuleshov-group/mdlm-owt, 169.6M) 코드 확인 결과:
- DiT backbone: `flash_attn` + `torch.cuda.amp` 하드코딩 → MPS 불가
- DiMamba backbone: `mamba_ssm` + `triton` → MPS 불가
- BD3-LM: 같은 코드베이스 → 동일하게 MPS 불가

→ CUDA GPU 없이는 MDLM/BD3-LM 직접 실행 불가능.
BERT iterative unmasking이 D3PM absorbing state와 이론적으로 동치이므로
현 단계에서는 충분한 대안.

### 향후 가능성 (CUDA GPU 접근 시)
- MDLM(169.6M): 진짜 discrete diffusion 궤적과 BERT pseudo-diffusion 비교
- BD3-LM spectrum: block_size 변화에 따른 밀도 반응 연속 관측
